...
const createsemesterregistrationintodb = async (
  ...
  if (isthereanyrunningsemester) {
    throw new Error(
      ` your ${isthereanyrunningsemester.status} semester is already running , you cant enter another semester registration untill its end `,
    );
  }
  ...
  const issemesterregistered = await semesterregistraionmodel.findOne({
    academicsemester,
  });
...
const updateregesteredsemester = async (
  id: string,
  payload: Partial<Tsemesterregistration>,
) => {
  const istheresmesterexist = await semesterregistraionmodel.findById({
    _id: id,
  });
  ...
  const result = await semesterregistraionmodel.findOneAndUpdate(
    { _id: id },
    payload,
    { new: true },
  );
